 ...
 * @author User
 */
public class ValidatorHikingGroup implements IValidator<Object>{

    @Override
    public void validate(Object entity) throws CustomException {
        if (entity == null) {
            throw new CustomException(ErrorType.INVALID_INPUT_ERROR, "Hiking group must be selected!");
        }
        if (!(entity instanceof HikingGroup)) {
            throw new CustomException(ErrorType.INVALID_INPUT_ERROR, "Hiking group must be selected!");
        }
    }
