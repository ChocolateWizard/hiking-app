 ...
 * @author Despot
 */
public class ValidatorHikerFirstName implements IValidator<String>{

    @Override
    public void validate(String entity) throws CustomException {
        if (entity == null) {
            throw new CustomException(ErrorType.INVALID_INPUT_ERROR, "First name is mandatory!");
        }
        ...
        if (entity.trim().length() > upperBound) {
            throw new CustomException(ErrorType.INVALID_INPUT_ERROR, "First name must be less than " + upperBound + " characters!");
        }
    }
